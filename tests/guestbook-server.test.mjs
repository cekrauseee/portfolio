...
import {
  createGuestbookGet,
  createGuestbookPost,
} from "../src/features/guestbook/handler.ts";
import {
...
test("publication remembers the name, exposes only public fields, and replays a lost response once", async () => {
  ...
  assert.doesNotMatch(
    ...
    /Ana Silva|Gostei muito|submissionId/,
  );
});
...
test("retry keys cannot change content or republish a hidden message", async () => {
  ...
  const conflict = await state.post(
    request({ ...submission, message: "Other" }),
  );
...
test("cross-site, invalid and oversized requests cannot reach OpenAI or the database", async () => {
  const state = setup();
  assert.equal(
    (await state.post(request(submission, { origin: "https://evil.test" })))
      .status,
    403,
  );
  assert.equal(
    (await state.post(request({ ...submission, name: " " }))).status,
    400,
  );
  assert.equal(
    (await state.post(request({ ...submission, message: "x".repeat(501) })))
      .status,
    400,
  );
  assert.equal(
    (
      await readJson(
        request({ ...submission, message: "x".repeat(9000) }),
        "guestbook",
      )
    ).response.status,
    413,
...
test("guardrail refusal or failure never publishes; locks release on every outcome", async () => {
  ...
  assert.doesNotMatch(
    JSON.stringify(failedWrite.events),
    /private SQL details/,
  );
...
test("cursors preserve timestamp and UUID and reject malformed input before a query", async () => {
  ...
  assert.deepEqual(
    decodeGuestbookCursor(encodeGuestbookCursor(reference)),
    reference,
  );
  for (const value of [
    "",
    "%",
    "x".repeat(257),
    Buffer.from('{"id":"bad"}').toString("base64url"),
  ])
  ...
  assert.equal(
    (await get(new Request("https://portfolio.test/api/guestbook?cursor=bad")))
      .status,
    400,
  );
...
test("name cookies handle Unicode and malformed values without tracking identity", () => {
  ...
  for (const [cookie, name] of [
    ...
    assert.equal(
      rememberedGuestbookName(
        new Request("https://portfolio.test", { headers: { cookie } }),
      ),
      name,
    );
  }
...
test("OpenAI guardrail uses parsed schema, short prompt and closed failure handling", async () => {
  ...
  try {
    ...
    for (const approved of [true, false]) {
      const result = await guardGuestbookMessage(
        submission,
        "hashed-identity",
        {
          openai: {
