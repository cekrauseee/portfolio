...
function productionEnvironment(overrides = {}) {
  return {
    ...
    DATABASE_URL: 'postgresql://portfolio:secret@database.example.test/portfolio',
...
test('production environment validation requires complete critical configuration', () => {
  ...
  assert.throws(
    () => validateProductionEnvironment(productionEnvironment({ DATABASE_URL: '' })),
    /DATABASE_URL is required/,
  )
  assert.throws(
    () => validateProductionEnvironment(productionEnvironment({ ANON_SESSION_SECRET: 'short' })),
...
test('published notes configuration is optional but validated when present', () => {
  assert.deepEqual(
    validateProductionEnvironment(
      productionEnvironment({
        NOTES_REPOSITORY: 'fixture-owner/notes',
        NOTES_REF: 'main',
      }),
    ),
    { meetingNotificationEnabled: false },
  ...
  assert.throws(
    () => validateProductionEnvironment(productionEnvironment({ NOTES_REPOSITORY: 'notes' })),
    /NOTES_REPOSITORY must use the owner\/repository form/,
  )
})
...
test('Resend is optional but must be configured as a complete group', () => {
  ...
  assert.deepEqual(validateProductionEnvironment(productionEnvironment(configured)), {
