...
function leafEntries(value) {
  ...
  function visit(current, path) {
    ...
    assert.equal(typeof current, 'string', `Expected a string at ${path.join('.')}`)
...
test('all locale dictionaries have the same string structure and interpolation contracts', () => {
  ...
  // The current interface interpolates counts and limits; the removed project
  // page's image description was the only consumer of the old {name} token.
  for (const token of ['count', 'max']) {
...
test('page metadata uses the personal name as its shared site title', () => {
  for (const dictionary of Object.values(dictionaries)) {
    assert.equal(dictionary.site.title, 'henrique krause')
  }
})
