...
const fixture = JSON.parse(
  readFileSync(new URL('../fixtures/github-projects.json', import.meta.url), 'utf8'),
)
...
const scheduling = await import('../src/features/meeting-scheduling/schedule-meeting.ts')
const calendar = await import('../src/features/meeting-scheduling/google-calendar.ts')
const notification = await import('../src/features/meeting-scheduling/meeting-notification.ts')
const scheduler = await import('../src/features/meeting-scheduling/meeting-scheduler.tsx')
...
function assertCookie(result) {
  assert.match(result.headers.get('set-cookie') ?? '', /anon_session=session-cookie/)
}
...
test('fit guards avoid upstream calls and preserve the anonymous session', async () => {
  ...
  const blockedResult = await blocked(jsonRequest('/api/fit', { description: 'x' }))
  assert.equal(blockedResult.status, 403)
  assert.equal((await blockedResult.clone().json()).error.code, 'request_denied')
  ...
  assert.equal((await malformedResult.clone().json()).error.code, 'invalid_description')
  assertCookie(malformedResult)

  const limited = await createFitPost({
    protect: async () =>
      abuse.withSession(response(429, {}, { 'Retry-After': '17' }), 'session-cookie'),
    assessRoleFit: assess,
  })(jsonRequest('/api/fit', {}))
...
test('fit emits one privacy-safe wide event with provider metadata', async () => {
  ...
  assert.doesNotMatch(JSON.stringify(logs[0]), /Build private APIs|stable-test-identity/)
})
...
test('meeting endpoint binds a key to one canonical request and replays it', async () => {
  ...
  const first = await handler(jsonRequest('/api/meetings', body, { 'Idempotency-Key': 'same' }))
  const replay = await handler(jsonRequest('/api/meetings', body, { 'Idempotency-Key': 'same' }))
  const mismatch = await handler(
    jsonRequest('/api/meetings', { ...body, name: 'Grace' }, { 'Idempotency-Key': 'same' }),
  )
...
test('concurrent meeting requests cannot own the same operation or slot', async () => {
  ...
  const firstPromise = handler(jsonRequest('/api/meetings', body, { 'Idempotency-Key': 'race' }))
  await new Promise((resolve) => setTimeout(resolve, 0))
  const duplicate = await handler(jsonRequest('/api/meetings', body, { 'Idempotency-Key': 'race' }))
  assert.equal(duplicate.status, 429)
  assert.match(duplicate.headers.get('Retry-After'), /^\d+$/)

  const otherKey = await handler(jsonRequest('/api/meetings', body, { 'Idempotency-Key': 'other' }))
...
test('the browser keeps one idempotency key for the same canonical payload', async () => {
  ...
  const first = await scheduler.resolveMeetingIdempotency(payload, undefined, storage, createKey)
  const retry = await scheduler.resolveMeetingIdempotency(payload, first, storage, createKey)
  const reload = await scheduler.resolveMeetingIdempotency(payload, undefined, storage, createKey)
