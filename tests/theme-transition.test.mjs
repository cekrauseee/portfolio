...
function fixture(t) {
  ...
  class Element {
    ...
    constructor(parent) {
      ...
      this.style = {
        ...
        setProperty: (key, value, priority) => this.inline.set(key, [value, priority]),
  ...
  function color(element) {
    return (
      element.displayed ??
      (element.parent ? color(element.parent) : dark ? 'rgb(240, 240, 240)' : 'rgb(20, 20, 20)')
    )
  }
  const globals = {
    HTMLElement: Element,
...
test('inherited labels use the final palette and the same clock as their parent', (t) => {
  ...
  for (const animation of f.animations) {
    ...
    assert.equal(animation.element.style.getPropertyValue('transition-duration'), '420ms')
    assert.equal(animation.element.style.getPropertyPriority('transition-duration'), 'important')
  }
})
...
test('temporary transition styles are restored if the update throws', (t) => {
  ...
  for (const element of f.elements) {
    assert.deepEqual([...element.inline], [['transition-duration', ['420ms', 'important']]])
  }
})
