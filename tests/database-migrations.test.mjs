...
test('the generic Drizzle setup remains available', () => {
  ...
  assert.equal(packageJson.scripts['db:check'], 'node scripts/check-database.mjs migrations')
  assert.equal(packageJson.scripts['db:verify'], 'node scripts/check-database.mjs schema')
...
test('guestbook has a migration chain and production delivery gates', () => {
  ...
  for (const workflow of ['.github/workflows/ci.yml', '.github/workflows/reconcile-projects.yml']) {
    const source = read(workflow)
    assert.match(source, /DATABASE_URL: \$\{\{ secrets\.DATABASE_URL_UNPOOLED \}\}/)
    assert.ok(
      source.indexOf('npm run db:migrate') < source.indexOf('name: Trigger Vercel deploy hook'),
    )
    assert.ok(
      source.indexOf('npm run db:verify') < source.indexOf('name: Trigger Vercel deploy hook'),
    )
  }
})
