...
import { createReadingCamera } from '../src/features/notes/reading-layout.ts'
import {
  animateChangedText,
...
test('project text inside buttons is included, preference controls stay still', (t) => {
  ...
  const preference = element('span', element('button', element('fieldset', root)), 'português')
  const nodes = [button, description, meta, preference].map((parentElement) => ({
    parentElement,
    textContent: parentElement.textContent,
  }))
  let index = -1
  const originalDocument = Object.getOwnPropertyDescriptor(globalThis, 'document')
  const originalFilter = Object.getOwnPropertyDescriptor(globalThis, 'NodeFilter')
  ...
  t.after(() => {
    ...
    if (originalFilter) Object.defineProperty(globalThis, 'NodeFilter', originalFilter)
...
test('translated text starts on the native clock, once per nested branch', (t) => {
  ...
  assert.deepEqual(animations[0].keyframes, [{ opacity: 0.325 }, { opacity: 0.5 }])
...
test('layout and text play together and release clipping after the longer movement', async (t) => {
  ...
  assert.equal(f.animations[0].options.duration, motion.duration.feedback)
  assert.equal(f.animations[1].options.duration, motion.duration.settle)
  f.frame()
...
test('expanded translations reveal existing and inserted copy once in final geometry', (t) => {
  ...
  for (const [oldHeight, inset] of [
    ...
    const animations = animateChangedText(f.root, before, after, new Map([[article, oldHeight]]))
    assert.equal(animations.length, 1)
    ...
    assert.equal(animations[0].options.duration, motion.duration.settle)
  }
...
test('expanded reveal leaves room for a preceding summary that gains lines', (t) => {
  ...
  const article = element('article:has(> h3 > button[aria-expanded=true])', f.root)
...
test('focused note translations share one header anchor and fade without footer-based movement', async (t) => {
  const f = lifecycleFixture(t)
  const main = element('main')
  f.root.parentElement = main
  main.scrollTop = 100
  ...
  t.mock.method(globalThis, 'getComputedStyle', () => ({ opacity: '1', overflowY: 'auto' }))
  const camera = createReadingCamera(main)
  const article = element('article', f.root)
  article.animate = f.text.animate
  const header = element('h3', article)
  let headerDocumentTop = 400
  header.getBoundingClientRect = () => ({ top: headerDocumentTop - main.scrollTop })
  article.querySelector = () => header
  f.root.querySelector = (selector) => (selector.includes('data-note-reading') ? article : null)
  for (const [locale, translatedTop, text] of [
    ['pt', 650, 'uma nota com título maior'],
    ['ja', 310, '読み上げるノート'],
    ['en', 510, 'a spoken note'],
  ]) {
    const request = requestLocaleTransition(f.root, locale)
    headerDocumentTop = translatedTop
    f.text.textContent = text
    const cleanup = playLocaleTransition(f.root, locale)
    assert.equal(header.getBoundingClientRect().top, 148)
    assert.equal(
      camera.isExpectedScroll(),
      true,
      'locale repositioning must not count as manual scroll',
    )
    const animation = f.animations.at(-1)
    assert.deepEqual(animation.keyframes, [{ opacity: 0 }, { opacity: 1 }])
    ...
    animation.finish()
    await new Promise(setImmediate)
    assert.equal(getLocaleTransition(locale), undefined)
    cleanup()
