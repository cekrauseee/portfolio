...
const { createInMemoryRedisAdapter } = await import('./support/in-memory-redis.mjs')
const resetInMemoryRedis = () => protection.setRedisAdapterForTests(createInMemoryRedisAdapter())
resetInMemoryRedis()
...
test('readJson enforces content type, malformed JSON, and byte limits', async () => {
  assert.equal(
    (await protection.readJson(request('{}', { 'content-type': 'text/plain' }), 'fit')).response
      .status,
    400,
  )
  assert.equal((await protection.readJson(request('{', {}), 'fit')).response.status, 400)
  assert.equal(
    (
...
test('production Redis errors fail closed with a session and never call upstream', async () => {
  ...
  try {
    const handler = createFitPost({
      protect: (operation, requestValue) => protection.protect(operation, requestValue),
      assessRoleFit: async () => {
        upstreamCalls += 1
        return 'must not run'
      },
