...
test("guestbook message merges keep local prepends and avoid duplicates", () => {
  assert.deepEqual(appendUniqueMessages([newer], [newer, older]), [
    newer,
    older,
  ]);
...
test("guestbook submission id is reused only for the same payload", () => {
  ...
  const changed = resolveGuestbookSubmission(
    "ana",
    "hello again",
    retry,
    createId,
...
test("a delayed first page keeps concurrent posts in newest-first order", () => {
  ...
  assert.deepEqual(appendUniqueMessages([newer], [latest, newer, older]), [
    latest,
    newer,
    older,
  ]);
  assert.deepEqual(prependUniqueMessage([latest, newer], older), [
    latest,
    newer,
    older,
  ]);
