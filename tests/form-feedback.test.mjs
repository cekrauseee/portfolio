...
import {
  FieldFeedback,
  FormErrorFeedback,
} from "../src/components/form-feedback.tsx";
...
test("field feedback animates between hint and error rows", () => {
  ...
  assert.match(error, /duration-\[180ms\]/);
  assert.match(error, /duration-150/);
...
test("general errors keep a stable alert while their row expands", () => {
  const empty = renderToStaticMarkup(
    createElement(FormErrorFeedback, { message: "" }),
  );
  const error = renderToStaticMarkup(
    createElement(FormErrorFeedback, { message: "Try again" }),
  );
