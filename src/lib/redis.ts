...
export function resolveRedisCredentials(environment: RedisEnvironment = process.env) {
  const url = environment.KV_REST_API_URL?.trim()
  const token = environment.KV_REST_API_TOKEN?.trim()
  return url && token ? { url, token } : undefined
}

export function resolveRedisConfiguration(environment: RedisEnvironment = process.env) {
