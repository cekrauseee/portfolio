...
export function ThemeProvider({
  ...
  const [preference, setPreferenceState] = useState(() => {
    ...
    return readThemeCookie() ?? parseThemePreference(document.documentElement.dataset.theme)
  })
  const [isDark, setIsDark] = useState(() => {
    ...
    const currentPreference =
      readThemeCookie() ?? parseThemePreference(document.documentElement.dataset.theme)
    return (
      currentPreference === 'dark' ||
      (currentPreference === 'system' && window.matchMedia(THEME_MEDIA_QUERY).matches)
    )
  })
