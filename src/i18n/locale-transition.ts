import { repositionReadingViewport } from '@/features/notes/reading-layout'
import { motion } from '@/lib/motion'
import { getViewportScroller } from '@/lib/viewport-scroll'
import { readingHeaderScrollDelta } from '@/features/notes/reading'
import type { Locale } from '@/i18n/config'
import { animateLocaleLayout, expandedArticle, readLocaleLayout } from '@/i18n/locale-layout'
...
export function readTextElements(root: HTMLElement) {
  ...
  while (walker.nextNode()) {
    ...
    if (
      ...
      element.closest('fieldset, input, textarea, select, script, style, svg, [aria-hidden="true"]')
    ) {
      continue
    }
...
export function animateChangedText(
  ...
  const candidates = new Set(
    [...currentText]
      ...
      .map(([element]) => element.closest<HTMLElement>(expandedArticle) ?? element),
  )
  ...
  const targets = [...candidates]
    ...
    .map((element) => {
      ...
      if (expanded) {
        ...
        if (sibling && sibling instanceof HTMLElement && articleHeights.has(sibling)) {
          growth += Math.max(
            0,
            sibling.getBoundingClientRect().height - articleHeights.get(sibling)!,
          )
        }
      }
      return {
  ...
  return targets.map(({ element, opacity, expanded, growth }) => {
    ...
    const animation = element.animate(
      ...
      {
        duration: expanded ? motion.duration.settle : motion.duration.feedback,
...
export function playLocaleTransition(root: HTMLElement, locale: Locale) {
  ...
  const readingArticle = root.querySelector<HTMLElement>('article[data-note-reading="true"]')
  const readingHeader = readingArticle?.querySelector<HTMLElement>(
    ':scope > [data-note-header], :scope > h3',
  )
  if (readingArticle && readingHeader) {
    const changed = [...readTextElements(root)].some(
      ([element, text]) => request.text.get(element) !== text,
    )
    if (!changed) return
    const scroller = getViewportScroller(readingHeader)
    const documentScroller = scroller === document.scrollingElement
    const viewportTop = documentScroller
      ? (window.visualViewport?.offsetTop ?? 0)
      : scroller.getBoundingClientRect().top
    const viewportHeight = documentScroller
      ? (window.visualViewport?.height ?? window.innerHeight)
      : scroller.clientHeight
    // This runs in a layout effect: commit the translated header to the same
    // viewport anchor before paint, then reveal the new content as one block.
    repositionReadingViewport(
      scroller,
      scroller.scrollTop +
        readingHeaderScrollDelta(
          readingHeader.getBoundingClientRect().top,
          viewportTop,
          viewportHeight,
        ),
    )
    if (
      ...
      typeof readingArticle.animate !== 'function'
    ) {
      finishLocaleTransition(request)
      return
    }
    const animation = readingArticle.animate([{ opacity: 0 }, { opacity: 1 }], {
