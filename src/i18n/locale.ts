...
function readHeader(source: HeaderSource, name: string) {
  ...
  const entry = Object.entries(source).find(([key]) => key.toLowerCase() === expected)
...
export function getDeploymentCountry(headers: HeaderSource) {
  return readHeader(headers, 'x-vercel-ip-country') ?? readHeader(headers, 'cf-ipcountry')
}
  ...
  const candidates = value
    ...
    .filter(
      (candidate): candidate is typeof candidate & { range: string } =>
        Boolean(candidate.range) && candidate.quality > 0 && candidate.quality <= 1,
    )
...
export function resolveLocale({ cookie, country, acceptLanguage }: LocaleResolutionInput): Locale {
  if (isLocale(cookie)) {
    return cookie
  }

  return localeFromCountry(country) ?? parseAcceptLanguage(acceptLanguage) ?? defaultLocale
}
