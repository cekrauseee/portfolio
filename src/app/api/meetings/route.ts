import type { MeetingErrorCode, MeetingErrorPayload } from '@/features/meeting-scheduling/errors'
import {
...
export function createMeetingsPost(overrides: Partial<MeetingsDependencies> = {}) {
  const dependencies = { ...defaultDependencies, ...overrides }

  return async function POST(request: Request) {
    const operation = createOperationEvent(request, 'meeting_scheduling', dependencies.logOperation)
    ...
    try {
      response = await handleMeetingsPost(request, dependencies, telemetry, operation.operationId)
    } catch (error) {
      telemetry.stage = 'handler'
      telemetry.outcome = 'failed'
      telemetry.error = safeErrorDetails(error)
      response = meetingErrorResponse('schedule_failed', operation.operationId, 502)
    }
...
async function handleMeetingsPost(
  ...
  const respond = (response: Response, outcome: string, stage = telemetry.stage) => {
    telemetry.outcome = outcome
    telemetry.stage = stage
    return response
  }

  const protection = await dependencies.protect('meetings', request, (diagnostic) => {
    telemetry.protection = diagnostic
  })
  if (protection instanceof Response) {
    const code = protectionErrorCode(protection.status)
    return respond(
      meetingErrorResponse(code, operationId, protection.status, protection.headers),
      code,
    )
  }
  ...
  try {
    ...
    const idempotencyOwner = await dependencies.acquire(idempotencyKey, LOCK_TTL_SECONDS)
    if (!idempotencyOwner) {
      return respond(
        withSession(
    ...
    try {
      ...
      const replay = await dependencies.readDedupe<unknown>(`meeting:${idempotencyDigest}`)
      if (replay !== null) {
        ...
        return respond(replayResponse(replay, protection.sessionCookie), 'completed', 'complete')
      }
      ...
      const activeOwner = await dependencies.acquire(activeKey, LOCK_TTL_SECONDS)
      if (!activeOwner) {
        return respond(
          withSession(
            meetingErrorResponse('rate_limited', operationId, 429, {
      ...
      try {
        ...
        try {
          ...
          await releaseBestEffort(dependencies.release, slotKey, slotOwner, telemetry)
        }
      } finally {
        await releaseBestEffort(dependencies.release, activeKey, activeOwner, telemetry)
      }
    } finally {
      await releaseBestEffort(dependencies.release, idempotencyKey, idempotencyOwner, telemetry)
    }
  } catch (error) {
    telemetry.error = safeErrorDetails(error)
    ...
    if (error instanceof MeetingConflictError) {
      return respond(
        withSession(meetingErrorResponse('conflict', operationId, 409), protection.sessionCookie),
        'conflict',
      )
    }
...
function meetingErrorResponse(
  ...
  return Response.json({ error: { code, operationId } } satisfies MeetingErrorPayload, {
    status,
    headers,
  })
}
...
function replayResponse(replay: MeetingDedupeRecord, sessionCookie: string | undefined) {
  return withSession(
    Response.json(
      {
        ok: true,
