...
export function createFitPost(overrides: Partial<FitDependencies> = {}) {
  ...
  return async function POST(request: Request) {
    const operation = createOperationEvent(request, 'fit_assessment', dependencies.logOperation)
    ...
    let lock: { acquired: boolean; release_error?: SafeErrorDetails } | undefined
    let assessment:
      | {
    ...
    let guardrail:
      | {
          ...
          model: string
          request_id?: string
          failure?: Extract<RoleFitGuardrailResult, { status: 'failed' }>['failure']
        }
      | undefined
    ...
    try {
      const protection = await dependencies.protect('fit', request, (diagnostic) => {
      ...
      if (!description) {
        return complete(
          errorResponse('invalid_description', 400, undefined, protection.sessionCookie),
          'invalid_description',
        )
      }
      ...
      if (!process.env.OPENAI_API_KEY?.trim()) {
        return complete(
          errorResponse('service_unavailable', 503, undefined, protection.sessionCookie),
          'service_unavailable',
        )
      }
      ...
      if (!lockOwner) {
        return complete(
          errorResponse('rate_limited', 429, { 'Retry-After': '30' }, protection.sessionCookie),
          'rate_limited',
        )
      }
      ...
      try {
        ...
        if (guardrailResult.status === 'failed') {
          ...
          response = errorResponse('description_rejected', 422, undefined, protection.sessionCookie)
      ...
      if (error instanceof ProtectionUnavailableError) {
        return complete(
          errorResponse('service_unavailable', 503, { 'Retry-After': '30' }, sessionCookie),
          'service_unavailable',
          error,
        )
      }
