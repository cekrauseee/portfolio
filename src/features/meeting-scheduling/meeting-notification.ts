...
export async function sendMeetingNotification(
  ...
  const configuration = options.configuration ?? resolveMeetingNotificationConfiguration()
  if (!configuration) {
    return false
  }
    ...
    resend.emails.send(
      {
        ...
        text: [
          ...
          ...(input.calendarLink ? ['', `Open the calendar event: ${input.calendarLink}`] : []),
        ].join('\n'),
      },
...
function withTimeout<T>(promise: Promise<T>, timeoutMs: number) {
  ...
  const deadline = new Promise<never>((_, reject) => {
    timeout = setTimeout(() => reject(new Error('Meeting notification timed out.')), timeoutMs)
  })
