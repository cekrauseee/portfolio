...
function calendarClient() {
  ...
  const auth = new google.auth.OAuth2(configuration.clientId, configuration.clientSecret)
 ...
 */
export async function findMeetingEvent(input: MeetingEventInput): Promise<MeetingEventLookup> {
...
export async function hasCalendarConflict(start: Date, end: Date, config = getCalendarConfig()) {
...
export async function createMeetingEvent(input: {
  ...
  try {
    ...
    if (existing.status === 'match') {
      ...
      throw new MeetingEventMismatchError('Google returned a conflicting event for this meeting.')
    }
  }
  ...
  return {
    id: currentEvent.id,
    meetLink:
      currentEvent.conferenceData?.entryPoints?.find((entry) => entry.entryPointType === 'video')
        ?.uri ?? undefined,
    calendarLink: currentEvent.htmlLink ?? undefined,
    replayed,
  }
...
function matchesMeetingEvent(
  ...
  return (
    ...
    (event.attendees ?? []).some((candidate) => normalizeEmail(candidate.email) === attendee) &&
...
function isGoogleNotFound(error: unknown) {
  ...
  return [value.code, value.status, value.statusCode, value.response?.status].some(
...
function isGoogleConflict(error: unknown) {
  ...
  return [value.code, value.status, value.statusCode, value.response?.status].some(
    (candidate) => Number(candidate) === 409,
  )
}
