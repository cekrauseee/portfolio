...
export function Guestbook({
  ...
  useEffect(() => {
    ...
    const frame = window.requestAnimationFrame(() => {
      if (
        activeAction === "guestbook" &&
        composerOpen &&
        !messageRef.current?.disabled
      ) {
  ...
  const loadInitial = useCallback(async () => {
    ...
    try {
      ...
      const parsed = response.ok
        ? GuestbookPageResponseSchema.safeParse(data)
        : undefined;
      if (!parsed?.success) {
  ...
  async function loadMore() {
    if (messages.length > visibleCount) {
      const revealed = messages.slice(
        visibleCount,
        visibleCount + GUESTBOOK_PAGE_SIZE,
      );
  ...
  function resolveSubmission(payloadName: string, payloadMessage: string) {
    const next = resolveGuestbookSubmission(
      payloadName,
      payloadMessage,
      submission.current,
    );
  ...
  async function handleSubmit(event: SubmitEvent<HTMLFormElement>) {
    ...
    const nextMessageError = !trimmedMessage
      ? "required"
      : message.length > MAX_GUESTBOOK_MESSAGE_LENGTH ||
          message.includes("\u0000")
        ? "invalid"
        : null;
  ...
  const inputClassName = (hasError: boolean) =>
    `w-full min-w-0 border bg-black/[0.035] px-4 py-2.5 [font-family:inherit] text-base normal-case leading-6 transition-[background-color,border-color,outline-color] duration-150 ease-out placeholder:text-black/45 motion-reduce:transition-none dark:bg-white/[0.04] dark:placeholder:text-white/45 ${focusVisibleClassName} ${
      hasError
  ...
  return (
    <div
      ...
      data-action-layout-pending={
        loadState === "idle" || loadState === "loading"
      }
    >
      <p className="mb-5 text-pretty text-black/65 dark:text-white/70">
        {dictionary.publicNotice}
      </p>
      ...
      <div>
        <div
          ...
          className={`grid transition-[grid-template-rows] duration-[180ms] ease-out motion-reduce:transition-none ${loadState === "loading" || loadState === "error" ? "grid-rows-[1fr]" : "grid-rows-[0fr]"}`}
        >
          <div className="min-h-0 overflow-hidden">
            <div
              className={`pb-4 transition-[opacity,translate] duration-150 ease-out motion-reduce:transition-none ${loadState === "loading" || loadState === "error" ? "translate-y-0 opacity-100" : "-translate-y-0.5 opacity-0"}`}
            >
              {loadState === "error" ? (
        ...
        <div
          ...
          className={`grid transition-[grid-template-rows,opacity] duration-180 ease-out motion-reduce:transition-none ${loadState === "loaded" && messages.length === 0 ? "grid-rows-[1fr] opacity-100" : "grid-rows-[0fr] opacity-0"}`}
        >
          <div className="min-h-0 overflow-hidden">
            <p className="pb-4 text-black/60 dark:text-white/65">
              {dictionary.empty}
            </p>
          </div>
        </div>
        ...
        <ol
          ...
          className={`transition-[filter,opacity] duration-[240ms] ease-out motion-reduce:transition-none ${composerOpen ? "opacity-35 blur-[1.5px]" : "blur-0 opacity-100"}`}
        >
          {messages.map((entry, index) => {
            ...
            return (
              <li
                ...
                inert={collapsed}
                className={`grid transition-[grid-template-rows] duration-[180ms] ease-out motion-reduce:transition-none ${entering || collapsed ? "grid-rows-[0fr]" : "grid-rows-[1fr]"}`}
                key={entry.id}
              >
                <div className="min-h-0 overflow-hidden">
                  <div
                    className={`pb-6 transition-[opacity,translate] duration-150 ease-out motion-reduce:transition-none ${entering || collapsed ? "-translate-y-0.5 opacity-0" : "translate-y-0 opacity-100"}`}
                  >
        ...
        <div
          ...
          className={`grid transition-[grid-template-rows,opacity] duration-180 ease-out motion-reduce:transition-none ${!composerOpen && (canRevealMore || canCollapse) ? "grid-rows-[1fr] opacity-100" : "grid-rows-[0fr] opacity-0"}`}
        >
          <div className="min-h-0 overflow-x-visible overflow-y-clip">
            <div className="pt-1">
              ...
              <div
                ...
                className={`grid transition-[grid-template-rows] duration-[180ms] ease-out motion-reduce:transition-none ${loadMoreState === "error" ? "grid-rows-[1fr]" : "grid-rows-[0fr]"}`}
              >
                <div className="min-h-0 overflow-hidden">
                  <p
                    className={`pt-2 text-red-700 transition-[opacity,translate] duration-150 ease-out motion-reduce:transition-none dark:text-red-400 ${loadMoreState === "error" ? "translate-y-0 opacity-100" : "-translate-y-0.5 opacity-0"}`}
                  >
      ...
      <div className="mt-5" ref={rootRef}>
        ...
        <section
          ...
          className={`grid transition-[grid-template-rows,opacity] duration-[400ms,160ms] ease-[cubic-bezier(0.4,0,0.2,1),ease-out] motion-reduce:transition-none ${composerOpen ? "grid-rows-[1fr] opacity-100" : "pointer-events-none grid-rows-[0fr] opacity-0"}`}
          id="guestbook-composer"
          inert={composerOpen ? undefined : true}
        >
          <div className="min-h-0 overflow-x-visible overflow-y-clip">
            <div
              className={`origin-top-left transition-transform duration-[400ms] ease-[cubic-bezier(0.2,0.9,0.3,1.15)] motion-reduce:transform-none motion-reduce:transition-none ${composerOpen ? "translate-y-0 scale-100" : "translate-y-2 scale-[0.98]"}`}
            >
              <form
              ...
              >
                <div>
                  <label
                    className={`text-[0.8125rem] font-medium transition-colors duration-150 ease-out motion-reduce:transition-none ${nameErrorMessage ? "text-red-700 dark:text-red-400" : ""}`}
                  ...
                  <input
                    aria-describedby={
                      nameErrorMessage ? "guestbook-name-error" : undefined
                    }
                  ...
                  <FieldFeedback
                    ...
                    errorId="guestbook-name-error"
                  />
                </div>

                <div>
                  <label
                    className={`text-[0.8125rem] font-medium transition-colors duration-150 ease-out motion-reduce:transition-none ${messageErrorMessage ? "text-red-700 dark:text-red-400" : ""}`}
                  ...
                  <FieldFeedback
                    ...
                    errorId="guestbook-message-error"
                  />
                </div>
              ...
              <div
                aria-hidden={!success}
                className={`grid transition-[grid-template-rows] duration-[180ms] ease-out motion-reduce:transition-none ${success ? "grid-rows-[1fr]" : "grid-rows-[0fr]"}`}
              >
                <div className="min-h-0 overflow-hidden">
                  <p
                    className={`pt-5 text-black/75 transition-[opacity,translate] duration-150 ease-out motion-reduce:transition-none dark:text-white/85 ${success ? "translate-y-0 opacity-100" : "-translate-y-0.5 opacity-0"}`}
                  >
                    {dictionary.success}
                  </p>
                </div>
