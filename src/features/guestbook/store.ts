...
export function publicGuestbookMessage(row: StoredGuestbookMessage): GuestbookMessage {
  return {
    id: row.id,
    name: row.name,
    message: row.message,
...
export async function listGuestbookMessages(cursor: GuestbookCursor | null) {
  ...
  const messages = rows.slice(0, GUESTBOOK_PAGE_SIZE).map(publicGuestbookMessage)
  return {
    messages,
    nextCursor:
...
export async function insertGuestbookMessage(input: GuestbookSubmission, submissionKey: string) {
  const [row] = await requireDatabase()
