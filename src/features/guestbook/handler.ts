import { acquire, digest, protect, readJson, release, withSession } from '@/lib/abuse-protection'
import { createOperationEvent, writeOperationEvent } from '@/lib/operation-event'
...
import { guardGuestbookMessage, GUESTBOOK_GUARDRAIL_MODEL } from './guard-message'
...
export function createGuestbookGet(overrides: Partial<Dependencies> = {}) {
  ...
  return async (request: Request) => {
    const operation = createOperationEvent(request, 'guestbook_read', dependencies.logOperation)
    ...
    if (value !== null && !cursor) {
      return privateResponse(
        operation.complete(Response.json({ error: { code: 'invalid_cursor' } }, { status: 400 }), {
...
export function createGuestbookPost(overrides: Partial<Dependencies> = {}) {
  const dependencies = { ...defaultDependencies, ...overrides }
  return async (request: Request) => {
    const operation = createOperationEvent(request, 'guestbook_publish', dependencies.logOperation)
    let stage = 'origin'
    let sessionCookie: string | undefined
    let lockOwner: string | false = false
    let lockKey = ''
    const details: Record<string, unknown> = {}
    const errorResponse = (code: GuestbookErrorCode, status: number, headers?: HeadersInit) =>
      withSession(
        Response.json({ error: { code, operationId: operation.operationId } }, { status, headers }),
        sessionCookie,
      )
    const complete = (response: Response, outcome: string) =>
      privateResponse(operation.complete(response, { ...details, outcome, stage }))
    ...
    try {
      ...
      stage = 'protection'
      const protection = await dependencies.protect('guestbook', request, (diagnostic) => {
        ...
        const code =
          protection.status === 403
            ...
            : protection.status === 429
              ? 'rate_limited'
              : 'service_unavailable'
        return complete(errorResponse(code, protection.status, protection.headers), code)
      }
      sessionCookie = protection.sessionCookie
      stage = 'validation'
      const parsed = await dependencies.readJson(request, 'guestbook')
      if (parsed.response) {
        return complete(errorResponse('invalid_message', parsed.response.status), 'invalid_message')
      }
      const result = GuestbookSubmissionSchema.safeParse(parsed.body)
      if (!result.success) {
        return complete(errorResponse('invalid_message', 400), 'invalid_message')
      }
      ...
      const submissionKey = dependencies.digest(`guestbook-submission:${input.submissionId}`)
      stage = 'lookup'
      const existing = await dependencies.findGuestbookSubmission(submissionKey)
      if (existing) {
      ...
      if (!lockOwner) {
        return complete(errorResponse('rate_limited', 429, { 'Retry-After': '30' }), 'rate_limited')
      }
      stage = 'guardrail'
      const startedAt = Date.now()
      const guardrail = await dependencies.guardGuestbookMessage(input, protection.identity)
      details.guardrail = {
        ...guardrail,
      ...
      if (guardrail.status === 'rejected') {
        ...
        stage = 'persist'
        const row = await dependencies.insertGuestbookMessage(input, submissionKey)
        if (!row) {
