...
const TRANSLATED_MARKDOWN_KEYS = ['description', 'highlights', 'metaDescription', 'summary']

/** Load environment files using the same precedence as Next.js. */
export function loadGithubProjectSyncEnv(projectDirectory = process.cwd(), mode = 'development') {
...
function parseProjectTranslationMarkdown(source, context) {
  ...
  return validateMarkdownTranslation(parsed.data, parsed.content, context, TRANSLATED_MARKDOWN_KEYS)
}
...
async function fetchWithTimeout(fetchImpl, url, options, timeoutMs) {
  ...
  try {
    ...
    if (controller.signal.aborted) {
      throw new Error(`GitHub request timed out after ${timeoutMs}ms for ${url}.`, { cause: error })
    }
    throw new Error(
...
async function getJson(fetchImpl, url, token, timeoutMs) {
  const response = await fetchWithTimeout(fetchImpl, url, { headers: apiHeaders(token) }, timeoutMs)
  let body
  try {
...
function repositoryIdentity(repo) {
  ...
  throw new Error('GitHub repository response omitted immutable id and canonical full_name.')
}
...
function validateRepository(repo) {
  ...
  if (typeof repo.private !== 'boolean') {
    throw new Error(`GitHub repository ${repo.name || 'unknown'} omitted boolean private metadata.`)
  }
  if (!nonEmptyString(repo.name) || !nonEmptyString(repo.default_branch)) {
    throw new Error('GitHub repository response omitted name or default branch.')
  }
...
export async function syncGithubProjects({
  ...
  while (true) {
    ...
    const url = nextUrl ? nextUrl : new URL(`/users/${encodeURIComponent(owner)}/repos`, apiBaseUrl)
    if (!nextUrl) {
    ...
    const { body, response } = await getJson(fetchImpl, url.toString(), token, timeoutMs)

    if (!Array.isArray(body)) {
      throw new Error(`GitHub repository response for page ${page} was invalid.`)
    }
    repos.push(...body)
  ...
  for (const rawRepo of repos) {
    ...
    const assetBaseUrl = projectAssetBaseUrl(owner, repo.name, repo.default_branch)
    ...
    if (markdownFile) {
      ...
      for (const [locale, filePath] of Object.entries(LOCALIZED_PROJECT_FILE_PATHS)) {
...
async function main() {
  ...
  if (process.env.PROJECTS_SYNC_SKIP === '1') {
    ...
    console.log('Skipping GitHub project reconciliation because PROJECTS_SYNC_SKIP=1.')
    return
  }
