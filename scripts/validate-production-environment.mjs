...
const OPTIONAL_RESEND_GROUP = ['RESEND_API_KEY', 'MEETING_OWNER_EMAIL', 'RESEND_FROM_EMAIL']
...
export function validateProductionEnvironment(environment = process.env) {
  ...
  const notesRepository = environment.NOTES_REPOSITORY?.trim()
  if (notesRepository && !/^[A-Za-z0-9_.-]+\/[A-Za-z0-9_.-]+$/.test(notesRepository)) {
    errors.push('NOTES_REPOSITORY must use the owner/repository form.')
  }

  const notesRef = environment.NOTES_REF?.trim()
  if (notesRef && /[\s\\]/.test(notesRef)) {
    errors.push('NOTES_REF must not contain whitespace or backslashes.')
  }
  ...
  const configuredResendValues = OPTIONAL_RESEND_GROUP.filter((name) => environment[name]?.trim())
  if (
    configuredResendValues.length > 0 &&
    configuredResendValues.length !== OPTIONAL_RESEND_GROUP.length
  ) {
    errors.push(`Configure all of ${OPTIONAL_RESEND_GROUP.join(', ')} or leave all three unset.`)
  }
  ...
  return {
    meetingNotificationEnabled: configuredResendValues.length === OPTIONAL_RESEND_GROUP.length,
  }
}
