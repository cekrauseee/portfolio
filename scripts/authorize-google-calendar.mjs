...
try {
  ...
  console.error(error instanceof Error ? error.message : 'Unable to read Google OAuth credentials.')
  process.exit(1)
}
...
const authorization = new Promise((resolveAuthorization, rejectAuthorization) => {
...
const server = createServer((request, response) => {
  ...
  if (!code) {
    ...
    failAuthorization(new Error('Google did not return an authorization code.'))
    return
  }
...
try {
  ...
  console.error(error instanceof Error ? error.message : 'Unable to authorize Google Calendar.')
  process.exitCode = 1
} finally {
  await close(server)
}
...
function writeEnvValue(path, name, value) {
  if (existsSync(path) && lstatSync(path).isSymbolicLink()) {
    throw new Error('Refusing to write GOOGLE_REFRESH_TOKEN through a symlink.')
  }
...
function withTimeout(promise, timeoutMs) {
  ...
  const result = Promise.race([
    ...
    new Promise((_, reject) => {
      timeout = setTimeout(() => reject(new Error('Google authorization timed out.')), timeoutMs)
    }),
  ])
