...
const localDatabaseUrl = 'postgres://portfolio:portfolio@127.0.0.1:5433/portfolio'
const obsoleteLocalDatabaseUrl = 'postgres://portfolio:portfolio@127.0.0.1:5432/portfolio'
...
The npm setup command selects Node.js 22.23.2 and npm 10.9.8 automatically.
Requires Docker with Compose and GITHUB_OWNER in .env.local.
...
ensureLocalEnvValue('ANON_SESSION_SECRET', () => randomBytes(48).toString('base64url'))
ensureLocalEnvValue('DATABASE_URL', () => localDatabaseUrl, [obsoleteLocalDatabaseUrl])
...
console.log('Activate Node.js 22 in your shell before running npm run dev.')
console.log('Or run: npx --yes --package=node@22.23.2 --package=npm@10.9.8 -- npm run dev')
...
function requireSupportedNode() {
  ...
  const supported =
    current[0] === minimumNodeVersion[0] && compareVersions(current, minimumNodeVersion) >= 0
  if (!supported) {
    console.error(
      `Node.js ${minimumNodeVersion.join('.')} or newer within the Node.js 22 release line is required. Current version: ${process.versions.node}. Run npm run setup to use the pinned runtime.`,
    )
    process.exit(1)
  }
}
...
function validateLocalConfiguration() {
  ...
  try {
    ...
    throw new Error('DATABASE_URL in .env.local must be a valid postgres:// URL.')
  }
